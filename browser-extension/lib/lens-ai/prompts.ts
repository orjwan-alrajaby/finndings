...
- needs — what the car has to do for their life. One need per real concern, labelled in their terms ("Keeping the kids occupied", "Feeling safer on busy roads", "Coping with winter"). Not one per Lens priority, and never a priority name as a label.
  ...
  - notInData: when the ideal thing they want isn't something FINN's data covers, name it plainly as the thing FINN doesn't publish ("how quiet the cabin is at speed", "how much room there is behind the front seats for a rear-facing seat", "which tyres are fitted", "how much rear legroom there is"), so Lens can say it isn't assuming it. Then look for related evidence that could still meet the underlying need.
